...
function Home({children} :any): JSX.Element {
    ...
    const settingCurrent = (newData:arrItem[]) => {
        ...
        console.log('inside settingcurrent newdata:', Object.keys(newData).length)
        if (newData !== undefined || newData !== null || newData !== []){
            ...
            console.log('inside settingcurrent arr:', Object.keys(arr).length)
        ...
        () => {
            if(passed1.current){
                ...
                if(!initalDataLoad.current){
                    ...
                    console.log('initial tbd set and settingcurrent(data)')
    ...
    const search = () => {
        if(inputValue !== ''){
            ...
            console.log('inside search arr:', Object.keys(arr).length)
            settingCurrent(arr)
         }
    }
    console.log('toBeDisplayed',  Object.keys(toBeDisplayed).length,
    ...
    const selectRegion = () =>{
        ...
        if(value !== 'all'){
            ...
            settingCurrent(data)
            setTobeDisplayed(data)
            console.log('on click fileter by region')
        }
    }
    search()
     return (
