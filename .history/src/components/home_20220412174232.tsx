...
function Home(): JSX.Element {
    ...
    useEffect(
        () => {
            let passed = false
            console.log("before if",passed)
            if(passed){
                localStorage.setItem('countries-data', JSON.stringify(data))
                console.log("inside if",passed)
                settingCurrent(data)
                console.log(currentCountries, currentCountriesSet.current)
                
            } 
            passed = true
            console.log("after if",passed)
           }
        , [data])
    
    console.log( 'data',data,'current countries', currentCountries,'set?', currentCountriesSet.current)
    return (
        <div>
            <div>
                donde en el mundo
