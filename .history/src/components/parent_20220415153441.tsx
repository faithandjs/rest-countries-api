...
import arrowBlack from './images/next.png'
import arrowWhite from './images/next (1).png'
...
const Parent: React.FC<React.ReactNode> =({children}) => {
    ...
    useEffect(
        () => {
            if(!passed.current){
                console.log(passed, 'lsd')
                if(  getLSD   ){
                    console.log(getLSD,JSON.parse(getLSD!).name !== '')
