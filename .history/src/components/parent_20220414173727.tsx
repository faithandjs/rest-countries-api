...
const Parent: React.FC<React.ReactNode> =({children}) => {
    ...
    const settingDetails = (obj:items) => {
        data.map((item, index) => {
            ...
            const objL = Object.keys(nativeNames).length;
            for (const [index, [key, value]] of Object.entries(nativeNames)) {
                console.log(`${key}: ${value.common}`, index );
              }
            console.log(index, active, typeof active.subregion)
            /*setDetails({
                flag: active.flags.svg,
                name: active.name.common,
                nativeName: active.name.nativeName,
                ...
                currencies: [],
                languages: [],
                borderCountries: active.borderCountries
            })*/
            }
        })
